...
class AdminLevelController extends Controller
{
    ...
    public function updateLevelQuestion(Request $request){
        $validator = Validator::make($request->all(),
            [
                'level_name'=>'required',
                'description'=>'required',
            ...
            if($validator->fails()){
                ...
                $data = $request->all();
                $lv = Level::find($data['id']);
                if(empty($lv)){
                    return response()->json(['status'=>404,'message'=>'Không tìm thấy độ khó câu hỏi!']);
                }
                else{
                    $lv->level_name = $data['level_name'];
                    $lv->description = $data['description'];
                    ...
                    $lv->point= $data['point'];
                    $lv->update();
                    return response()->json(['status'=>200,'message'=>'Cập nhật độ khó câu hỏi thành công!']);
                }
            }
    }

    public function deleteLevelQuestion($id){
        $lv = Level::find($id);
        // dd($tpq);
        if(empty($lv)){
            ...
            // dd($tpq);
            $lv->delete();
            return response()->json(['status'=>200,'message'=>'Xóa độ khó câu hỏi thành công!']);
        }
    }
}